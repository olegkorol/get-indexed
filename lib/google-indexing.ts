  ...
  for (const url of urls) {
    try {
      ...
      results.push({
        ...
        url,
        urlNotificationMetadata: (response.data as any)?.urlNotificationMetadata as IndexingStatusResponse ?? null,
      })
    } catch (error) {
      results.push({
        success: false,
 ...
 * @param credentials Service account credentials JSON
 */
export async function getUrlStatus(urls: string[], credentials: Record<string, any>) {
  const client = new JWT({
    email: credentials.client_email,
    key: credentials.private_key,
    scopes: ['https://www.googleapis.com/auth/indexing']
